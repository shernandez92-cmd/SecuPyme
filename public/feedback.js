(function() {
  ...
  function abrirModal() {
    ...
    overlay.innerHTML = `
      <div style="background:var(--morado-oscuro, #f0ebff);border:1px solid rgba(124,58,237,0.4);border-radius:4px;padding:40px;width:100%;max-width:480px;box-shadow:0 0 60px rgba(124,58,237,0.15);">
        ...
        <div style="margin-bottom:16px;">
          ...
          <input id="feedback-email" type="email" placeholder="correo@empresa.com" style="width:100%;background:var(--morado-oscuro,rgba(124,58,237,0.05));border:1px solid rgba(124,58,237,0.2);color:var(--texto,#1a0a2e);font-family:'Share Tech Mono',monospace;font-size:12px;padding:10px 12px;border-radius:2px;box-sizing:border-box;">
        </div>
        <div style="margin-bottom:24px;">
          ...
          <textarea id="feedback-mensaje" placeholder="Cuéntanos qué ocurrió..." rows="5" style="width:100%;background:var(--morado-oscuro,rgba(124,58,237,0.05));border:1px solid rgba(124,58,237,0.2);color:var(--texto,#1a0a2e);font-family:'Share Tech Mono',monospace;font-size:12px;padding:10px 12px;border-radius:2px;resize:vertical;box-sizing:border-box;"></textarea>
        </div>
  ...
  window.enviarFeedback = async function() {
    ...
    const btn = document.querySelector('#feedback-modal button:last-of-type');
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (btn) { btn.textContent = 'ENVIAR REPORTE'; btn.disabled = false; }
      }
    } catch (e) {
      status.style.display = 'block';
      status.style.color = '#ef4444';
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', crearFeedback);
  } else {
    crearFeedback();
  }
})();

window.abrirFeedbackModal = abrirModal;
